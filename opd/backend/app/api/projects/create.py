...
from ...schemas import (
    ProjectSchema, ProjectSchemaCreateRequest, UserUpdate
)
...
async def create(
    request: Request,
    body: ProjectSchemaCreateRequest,
    db_session: AsyncSession = Depends(get_db),
) -> Optional[ProjectSchema]: 
    await logger.write(
        f"{request.method} Request from {request.client.host}: {request.client.port} Handler /projects/create/"
    )
    ...
    async with connector.start() as administrator:
        project_id = await administrator.projects.create_model_from_schema(
            body, user_id=user['user_id']
        )
        user['projects_ids'].append(project_id)
