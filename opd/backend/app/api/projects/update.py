...
from ...schemas import (
    ProjectSchema, ProjectSchemaUpdateRequest, ProjectSchemaUpdate
)
...
async def update(
    request: Request,
    body: ProjectSchemaUpdateRequest,
    db_session: AsyncSession = Depends(get_db),
) -> Optional[ProjectSchema]: 
    await logger.write(
        f"{request.method} Request from {request.client.host}: {request.client.port} Handler /projects/update/"
    )
    ...
    async with connector.start() as administrator:
        await administrator.projects.update_model_from_schema(
            schema=ProjectSchemaUpdate(**body.model_dump()), 
